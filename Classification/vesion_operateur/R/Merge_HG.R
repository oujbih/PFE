...
merge_variableHG <- function(var_merge,regle=mean,regle2=mean,a=1,par_hours=F,Fevrier_DATA){
  if(var_merge=="SDP sans CPT"){
    ...
    for(i in 1:nrow(Jointure_par_CPT)){
      if(i==1){
        PM_HG_tmp <- Fevrier_DATA_CPT %>% filter(DATE>=as.Date("2020-01-02")&DATE<=Jointure_par_CPT$sortie_date[i+1])
      }else if (i==nrow(Jointure_par_CPT)){
        PM_HG_tmp <- Fevrier_DATA_CPT %>% filter(DATE>=Jointure_par_CPT$sortie_date[i-1])
      }else{
        PM_HG_tmp <- Fevrier_DATA_CPT %>% filter(DATE>=Jointure_par_CPT$sortie_date[i-1]&DATE<=Jointure_par_CPT$sortie_date[i+1])
      }
...
# microbenchmark(Jointure_par_PM2(regle = "mean", sans_cpt = F,a=1),times = 1)
# Jointure_par_PM_resl = Jointure_par_PM2(regle = "mean", sans_cpt = F,a=1)
# 
# My_data_Model_SDP_mean=merge_variableHG(input_var_merge,regle_jointure_value,regle_jointure_value2)
# 
# 
# My_data_Model_SDP_mean_hour1=merge_variableHG(input_var_merge,regle_jointure_value,regle_jointure_value2,a = 1,par_hours = T)
# 
# 
