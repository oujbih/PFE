...
X1 =scaled_train[1:14]
X2 = sample(0:127, 7000, replace=TRUE)

# create training response numbers
Y <- scaled_train$X40µm
...
a=array(c(1:3,4:6), c(3,2,2))


#
X1=as.matrix(t(as.matrix(scaled_train[,1])))
X2=as.matrix(t(as.matrix(scaled_train[,2])))
X3=as.matrix(t(as.matrix(scaled_train[,3])))
X4=as.matrix(t(as.matrix(scaled_train[,4])))
y= as.matrix(t(as.matrix(scaled_train[,23])))


x_train=array(c(X1,X2,X3,X4),dim = c(dim(X1),4))
